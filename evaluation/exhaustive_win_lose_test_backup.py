...
class ExhaustiveWinLoseTest:
    ...
    def _get_action_probabilities(self, board: np.ndarray) -> np.ndarray:
        """
        获取模型预测的动作概率分布
    ...
    def _visualize_probabilities(self, board: np.ndarray, probs: np.ndarray) -> str:
        """
        可视化动作概率分布
        ...
        for i in range(0, 9, 3):
            row_probs = []
            for j in range(i, i+3):
                if board[j] == 0:  # 只显示空位的概率
                    row_probs.append(f"{probs[j]:5.1%}")
                else:
                    row_probs.append("  -  ")

            lines.append(f"  {' | '.join(row_probs)}")
            if i < 6:
                lines.append(f"  ----------------")
    ...
    def test_winning_cases(self, test_cases: List[Tuple[np.ndarray, int, str]],
        ...
        if len(failed_cases) > 0 and not verbose:
            ...
            for i, (board, correct_action, predicted_action, desc) in enumerate(failed_cases[:max_display]):
                ...
                # 显示动作概率分布
                probs = self._get_action_probabilities(board)
                print(f"  动作概率分布:")
                print(self._visualize_probabilities(board, probs))
                print(f"  正确动作概率: {probs[correct_action]:.2%}")
                print(f"  预测动作概率: {probs[predicted_action]:.2%}")
    ...
    def test_losing_cases(self, verbose: bool = False, max_display: int = 5) -> Dict:
        """
        测试所有"一步防守"的情况
        重新生成合法的防守案例（对手差一步获胜，己方需要阻止）
        ...
        # 重新生成合法的防守案例
        defending_cases = self._generate_defending_cases()
        print(f"测试案例数: {len(defending_cases)}\n")

        print("开始测试模型...")
        correct = 0
        failed_cases = []

        for i, (board, correct_action, desc) in enumerate(defending_cases):
            predicted_action = self._predict_action(board, deterministic=True)
            ...
            if verbose and not is_correct:
                print(f"\n✗ [{i+1}/{len(defending_cases)}] {desc}")
                print(self._visualize_board(board, highlight_pos=correct_action))
                print(f"  正确: {correct_action}, 预测: {predicted_action}")

        accuracy = correct / len(defending_cases) * 100 if len(defending_cases) > 0 else 0

        print(f"\n结果:")
        print(f"  正确: {correct}/{len(defending_cases)} ({accuracy:.2f}%)")
        print(f"  错误: {len(defending_cases) - correct}")
